...
import { indiceSiguiente } from "@lib/array";
import { CUERPO_NOTICIA_PLACEHOLDER } from "@constants/noticias";
...
export default function NewsModal({
  ...
  // CONTROL DEL SCROLL DEL FONDO (guarda y restaura la posición exacta)
  useBodyScrollLock(isOpen);

  // NAVEGACIÓN POR TECLADO: Escape cierra, flechas cambian de noticia
  useEffect(() => {
    if (!isOpen) return;
    const handleEsc = (e: KeyboardEvent) => {
      const KEYS = ["Escape", "ArrowRight", "ArrowLeft"];
      if (e.key === KEYS[0]) onClose();
      if (e.key === KEYS[1]) {
        e.preventDefault();
        onNext();
      }
  ...
  // COMPARTIR NOTICIA: Web Share API o copiar URL al portapapeles
  const handleShare = async () => {
    if (!noticia) return;
    await compartir({ title: noticia.titulo, text: noticia.resumen });
  };
  ...
  const siguienteNoticia =
    todasLasNoticias[indiceSiguiente(indiceActual, todasLasNoticias.length)];
        ...
        <div className="noticias-modal__contenido p-0! relative flex flex-col h-full overscroll-none">
          ...
          <div className="p-5 md:p-6 pb-8">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-2 sm:gap-3">
              <span className="text-[0.625rem] sm:text-xs font-semibold uppercase tracking-widest text-accent">{noticia.categoria}</span>
            ...
            {/* Línea Divisoria */}
            <div className="h-px mb-4 bg-linear-to-r from-[rgba(90,58,158,0.4)] to-transparent"></div>

            <div className="flex flex-col gap-3 text-[0.875rem] sm:text-[0.9375rem] leading-relaxed text-white/75 min-w-0">
              <p className="wrap-break-word font-medium text-white/90">{noticia.resumen}</p>
              {CUERPO_NOTICIA_PLACEHOLDER.map((parrafo) => (
                <p key={parrafo} className="wrap-break-word">{parrafo}</p>
              ))}
            </div>
          </div>
          ...
          <div className="sticky bottom-0 z-20 w-full bg-[#2a1252] px-4 py-3 md:px-5 border-t border-[rgba(160,120,220,0.15)] shadow-[0_-10px_20px_rgba(26,15,48,0.7)] mt-auto">
            <button 
              className="flex flex-col justify-center px-3 py-2 rounded-md border border-[rgba(160,120,220,0.15)] bg-[rgba(90,58,158,0.08)] text-left cursor-pointer transition-all w-full hover:bg-[rgba(90,58,158,0.25)] hover:border-[rgba(160,120,220,0.4)]" 
              onClick={(e) => { e.stopPropagation(); onNext(); }}
            >
              <div className="flex items-center justify-between w-full">
                <span className="text-[0.5rem] font-bold uppercase tracking-widest text-accent">
                  Siguiente noticia
                </span>
                <div className="flex items-center gap-1 text-[0.5rem] text-white/40">
                  <span className="truncate max-w-20">{siguienteNoticia.categoria}</span>
                </div>
              </div>
